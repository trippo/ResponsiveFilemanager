...
(function($, Modernizr, image_editor)
{
    ...
    var FileManager = {
        contextActions: {
            copy_url: function($trigger)
            {
                ...
                alertDialog('Url ', 'mif-link',
                        '<div class="input-control text full-size success" data-role="input">'
                        + '<input id="url_text' + copy_count + '" type="text" value="' + encodeURL(m) + '" readonly/>' +
                        '<button id="copy-button' + copy_count + '" class="button success text-shadow" data-clipboard-target="url_text' + copy_count + '" data-clipboard-text="Copy Me!" title="copy">' +
                        +'</button>'
                        + '</div>');

                $('#copy-button' + copy_count).html('<span class="mif-share"></span> ' + $('#lang_copy').val());
                ...
                client.on("ready", function(readyEvent)
                {
                    ...
                    {
                        $('#copy-button' + copy_count).html('<span class="mif-thumbs-up"></span> ' + $('#ok').val());
                        $('#copy-button' + copy_count).attr('class', 'button disabled text-shadow');
                        copy_count++;
                    });
            ...
            duplicate: function($trigger)
            {
                ...
                showPrompt($('#lang_duplicate').val(), $('#cancel').val(), $('#ok').val(), function(name)
                {
                    if (name !== null)
                    {
                ...
                build: function($trigger)
                {
                                    ...
                                    ) && image_editor)
                    {
                        options.items.edit_img = {
                            name: '<span class="mif-image"></span>' + $('#lang_edit_image').val(),
                            icon: "",
                            disabled: false
                        };
                    }
                    ...
                    options.items.copy_url = {
                        name: '<span class="mif-link"></span> ' + $('#lang_show_url').val(),
                        disabled: false
                    };
                    // extract
                    ...
                    {
                        options.items.unzip = {
                            name: '<span class="mif-file-zip"></span> ' + $('#lang_extract').val(),
                            disabled: false
                        };
                    }
                    ...
                    if ($trigger.find('.img-precontainer-mini .filetype').hasClass('edit-text-file-allowed'))
                    {
                        options.items.edit_text_file = {
                            name: '<span class="mif-pencil"></span> ' + $('#lang_edit_file').val(),
                            disabled: false
                        };
                    }
                    ...
                    if (!$trigger.hasClass('directory') && $('#duplicate').val() == 1)
                    {
                        options.items.duplicate = {
                            name: '<span class="mif-stack3"></span> ' + $('#lang_duplicate').val(),
                            disabled: false
                        };
                    }
                    ...
                    {
                        options.items.copy = {
                            name: '<span class="mif-files-empty"></span> ' + $('#lang_copy').val(),
                            disabled: false
                        };
                        options.items.cut = {
                            name: '<span class="mif-cut"></span>' + $('#lang_cut').val(),
                            disabled: false
                        };
                    }
                    else if ($trigger.hasClass('directory') && $('#copy_cut_dirs_allowed').val() == 1)
                    {
                        ...
                        options.items.cut = {
                            name: '<span class="mif-cut" style="margin-right:25px;"></span>' + $('#lang_cut').val(),
                            icon: "cut",
                            disabled: false
                        };
                    }
                    ...
                    {
                        options.items.paste = {
                            name: '<span class="mif-clipboard"></span> ' + $('#lang_paste_here').val(),
                            disabled: false
                        };
                    }
                    ...
                    {
                        options.items.chmod = {
                            name: '<span class="mif-key"></span> ' + $('#lang_file_permission').val(),
                            disabled: false
                        };
                    }
                    ...
                    options.items.name = {
                        name: '<span class="mif-tag"></span> ' + $trigger.attr('data-name'),
                        disabled: true
                    };
                    if ($trigger.attr('data-type') == "img")
                    {
                        options.items.dimension = {
                            name: '<span class="mif-enlarge"></span> ' + $trigger.find('.img-dimension').html(),
                            disabled: true
                        };
                    }
                    if ($trigger.hasClass('directory')) {
                        options.items.size = {
                            name: '<span class="mif-justice"></span> ' + $trigger.find('.file-size').html() + " - " + $trigger.find('.nfiles').val() + " " + $('#lang_files').val() + " - " + $trigger.find('.nfolders').val() + " " + $('#lang_folders').val(),
                            disabled: true
                        };
                    } else {

                        options.items.size = {
                            name: '<span class="mif-justice"></span> ' + $trigger.find('.file-size').html(),
                            disabled: true
                        };
                    }
                    options.items.date = {
                        name: '<span class="mif-calendar"></span> ' + $trigger.find('.file-date').html(),
                        disabled: true
                    };
        ...
        bindGridEvents: function()
        {
            ...
            grid.on('click', '.modalAV', function(e)
            {
                ...
                if (_this.hasClass('audio'))
                {
                    ...
                    $('#previewAV .preview_icon').removeClass().addClass("preview_icon mif-music");
                }
                else
                {
                    bodyPreviewElement.css('height', '345px');
                    $('#previewAV .preview_icon').removeClass().addClass("preview_icon mif-film");
                }
                ...
                $.ajax({
                    ...
                    success: function(data)
                    {
                        ...
                        showDialog('#previewAV');
                        var path = _this.attr('data-url').split("/");//get file name
                        $('#preview_name').html(path[path.length - 1]);// put file name on title
            ...
            grid.on('click', '.file-preview-btn', function(e)
            {
                ...
                $.ajax({
                    ...
                    {
                        var path = _this.attr('data-url').split("/");    //get file name                                           
                        alertDialog($('#lang_preview').val() + " <strong>" + path[path.length - 1] + "</strong>", "mif-eye mif-ani-float mif-ani-slow", data);
            ...
            grid.on('click', '.rename-file', function()
            {
                ...
                showPrompt($('#rename').val(), $('#cancel').val(), $('#ok').val(), function(name)
                {
                    if (name !== null)
                    {
                        name = fix_filename(name);
            ...
            grid.on('click', '.delete-file', function()
            {
                ...
                var file_container = _this.parent().parent().parent();
                var file_title = file_container.find('h4');
                var namex = $.trim(file_title.text());
                //
                showConfirmDialog(_this.attr('title'), _this.attr('data-confirm') + ' <strong>' + namex + '</strong>', $('#cancel').val(), $('#ok').val(), function(result)
                {
                    if (result === true)
                    {
        ...
        makeFilters: function(js_script)
        {
            ...
            {
                $('.filters label').parent().removeClass("active");
                //$('.filters label').find('i').removeClass('icon-white');
                $('#ff-item-type-all').parent().addClass("active");
        ...
        makeSort: function(js_script)
        {
            ...
            $('input[name=radio-sort]').on('click', function()
            {
                ...
                labelElement.parent().removeClass("active");
                //labelElement.find('i').removeClass('icon-white');

                $('#filter-input').val('');

                liElement.parent().addClass("active");
                ...
                else
                {
                    if ($(this).is(':checked'))
                    {
                        $('.grid-main li').not('.' + li).hide(300);
                        $('.grid-main li.' + li).show(300);
                    }
                }
    ...
    $(document).ready(function()
    {
        ...
        {
            alertDialog($('#lang_about').val(), "mif-paw", '<div class="text-center"><br/><img src="img/logo.png" alt="responsive filemanager"/><br/><br/><p><strong>RESPONSIVE filemanager v.'
        ...
        {
            showPrompt($('#insert_folder_name').val(), $('#cancel').val(), $('#ok').val(), function(name)
            {
                if (name !== null && name !== "")
                {
                    name = fix_filename(name).replace('.', '');
        ...
        $('.view-controller a').on('click', function()
        {
            var _this = $(this);

            $('.view-controller a').parent().removeClass('active');
...
//            $('.tip-right').tooltip({placement: "right"});
            ...
            $('.tip,.tip-top,.tip-right,.tip-left').each(function() {
                var t = $(this);
//                t.data("hint-background", "bg-green").data("hint-mode", "2")
//                        .data("hint", t.attr('title'));
                t.data("hint", t.attr('title'));
                t.hint({hintPosition: "bottom", hintBackground: "bg-blue", hintMode: 2, hintColor: "#ffffff"});
            });
    ...
    function create_text_file()
    {
        ...
        var init_form = //'<label>'+ $('#lang_filename').val()+'</label>'+
                //'<span class="mif-file-empty prepend-icon"></span> ' +
                ' <div class="input-control text info">' +
                '<input type="text" id="create_text_file_name"  placeholder="' + $('#lang_filename').val() + '">' +
                '</div>'
                + '<br><div class="input-control textarea success full-size"><textarea id="textfile_create_area" style="width:100%;height:150px;"></textarea></div>';
        showConfirmDialog($('#lang_new_file').val(), init_form, $('#cancel').val(), $('#ok').val(), function()
        {
            ...
            if (newFileName !== null && newFileName !== "")
            {
                ...
                }).done(function(status_msg)
                {
                    if (status_msg != "")
                    {
                        alertDialog2($('#lang_new_file').val(), "mif-info", status_msg, $('#ok').val(), function(/*result*/)
                        {
                            setTimeout(function()
                            {
                                window.location.href = $('#refresh').attr('href') + '&' + new Date().getTime();
    ...
    function edit_text_file($trigger)
    {
        ...
        {
            showConfirmDialog($('#lang_edit_file').val() + ' <strong>' + $trigger.find('.name_download').val() + '</strong>', '<div class="input-control textarea full-size">' + init_content + '</div>', $('#cancel').val(), $('#ok').val(), function()
            {
                ...
                }).done(function(status_msg)
                {
                    if (status_msg !== "")
                    {
                        alertDialog($('#lang_info').val(), "mif-info", status_msg);
                    }
                });
            }, "mif-pencil");
        });
    ...
    function change_lang()
    {
        ...
        {
            showConfirmDialog($('#lang_lang_change').val(), '<div class="input-control select full-size info">' + init_msg + '</div>', $('#cancel').val(), $('#ok').val(), function()
            {
                // get new lang
                ...
                }).done(function(error_msg)
                {
                    if (error_msg !== "")
                    {
                        alertDialog($('#lang_info').val(), "mif-warning", error_msg);
                    }
                    else
                    {
                        setTimeout(function()
    ...
    function chmod($trigger)
    {
        ...
        {
            showConfirmDialog($('#lang_file_permission').val(), init_msg, $('#cancel').val(), $('#ok').val(),
                    function() {
                        // get new perm
                        ...
                        if (newPerm != '' && typeof newPerm !== "undefined")
                        {
                            ...
                            }).done(function(status_msg)
                            {
                                if (status_msg != "")
                                {
                                    alertDialog($('#lang_info').val(), "mif-warning", status_msg);
                                }
                            });
                        }
                    }, "mif-lock");
    ...
    {
        showConfirmDialog('Resposive File Manager', $('#lang_clear_clipboard_confirm').val(), $('#cancel').val(), $('#ok').val(), function(result)
        {
            if (result == true)
            {
                $.ajax({
    ...
    {
        showConfirmDialog('Resposive File Manager', $('#lang_paste_confirm').val(), $('#cancel').val(), $('#ok').val(), function(result)
        {
            if (result == true)
            {
                var folder_path;
...
// because of feedback and on error bahhhhh...
    ...
    {
        var width = $('.grid-main').width() + adding - 25;
